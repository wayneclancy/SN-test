...
#
# This is a simple alternative to using jenkins-cli.jar which may not be installed on the machine.
# Another option could be to download the hpi file in JENKINS_HOME/pluggins, unzip and parse META-INF/MANIFEST.MF and
# forefill all deps. This is a horrible method and would also require a jenkins reload.


if [ "$3" = "" ]
then
  echo "Usage: $0 <Jenkins_admin_username jenkins_admin_passowrd jenkins_pluggin version(optional)>"
  exit 1
fi
